    ...
    <div className="report-page">
      ...
      <div
        style={{
          ...
          borderRadius: 12, border: styles.border, background: styles.background,
        }}
      >
...
function ReportComp() {
  ...
  // ── check declared + complete + software + sequence ──
  useEffect(() => {
    ...
    http.get(`/auth/${userId}/user`)
      ...
      .then((data) => {
        ...
        setSoftwareUsed(!!data?.softwareUsed);     // ✅ NEW
        setNotInSequence(!!data?.notInSequence);   // ✅ NEW
      })
      .catch(() => {
        setIsDeclared(false);
        setIsComplete(true);
  ...
  // ── fetch report only if all clear ──
  const canShowReport = isDeclared && isComplete && !softwareUsed && !notInSequence;
  ...
  function highlightErrors(original = "", userText = "") {
    ...
    const align = [];
    while (i > 0 && j > 0) {
      if (oNorm[i-1] === uNorm[j-1]) { align.unshift({ ow: oWords[i-1], uw: uWords[j-1] }); i--; j--; }
      else if (dp[i-1][j] >= dp[i][j-1]) { align.unshift({ ow: oWords[i-1], uw: null }); i--; }
      else { align.unshift({ ow: null, uw: uWords[j-1] }); j--; }
    }
    ...
    return align.map(({ ow, uw }, k) => {
      if (ow && !uw) return <span key={k} className="error-red">({ow}) </span>;
      if (!ow && uw) return <span key={k} className="error-orange">{uw} </span>;
      if (stripPunct(ow).toLowerCase() !== stripPunct(uw).toLowerCase())
        return <span key={k} className="error-red">{uw} </span>;
      if (getPunct(ow) !== getPunct(uw))
        return <span key={k} className="error-blue">{uw} </span>;
      return <span key={k}>{uw} </span>;
    });
  }
  ...
  const handleConfirmDownload = async () => {
    ...
    sheet.addRow([]);
    const headerRow = sheet.addRow(["Page","Capital/Small Errors","Punctuation Errors","Missing/Extra Words","Spelling Errors","Total % Error"]);
    ...
    results.forEach((r, idx) => {
      ...
      row.eachCell((cell) => {
        ...
        cell.alignment = { horizontal: "center", vertical: "middle" };
        cell.border = { top:{style:"thin",color:{argb:"DDDDDD"}},left:{style:"thin",color:{argb:"DDDDDD"}},bottom:{style:"thin",color:{argb:"DDDDDD"}},right:{style:"thin",color:{argb:"DDDDDD"}} };
      });
      row.getCell(1).font = { bold: true };
    });
    sheet.addRow([]);
    const totalRow = sheet.addRow(["TOTAL % ERROR","","","","",totalErrorSum.toFixed(2)]);
    ...
    sheet.columns = [{width:18},{width:22},{width:22},{width:25},{width:20},{width:18}];
    const buffer = await workbook.xlsx.writeBuffer();
    saveAs(new Blob([buffer],{type:"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"}),`DATA_CONVERSION_Report_${new Date().toISOString().slice(0,10)}.xlsx`);
    setShowModal(false);
  };
    ...
    <div className="report-page">
      ...
      {results[selectedIndex] && (
        <div key={results[selectedIndex]._id} className="report-block">
          ...
          <div className="error-summary">
            ...
            <ul>
              <li><span className="color-box color-red"></span>Capital/Small: {results[selectedIndex].capitalSmall}</li>
              <li><span className="color-box color-blue"></span>Punctuation: {results[selectedIndex].punctuation}</li>
              <li><span className="color-box color-red"></span>Missing/Extra Word: {results[selectedIndex].missingExtraWord}</li>
              <li><span className="color-box color-red"></span>Spelling: {results[selectedIndex].spelling}</li>
              <li><b>Total % Error: {Number(results[selectedIndex].totalErrorPercentage || 0).toFixed(2)}%</b></li>
            </ul>
          </div>
        </div>
      )}
      ...
      >
        ...
        <table className="modal-table">
          <thead>
            <tr>
              <th>Page</th><th>Capital/Small</th><th>Punctuation</th>
              <th>Missing/Extra Word</th><th>Spelling</th><th>Total % Error</th>
            </tr>
          </thead>
          <tbody>
            {results.map((r) => (
